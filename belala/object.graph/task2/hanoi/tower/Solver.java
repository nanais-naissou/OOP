...
public class Solver {
   ...
   */
  public Solver(Board board, PrintStream ps) {
    this.board = board;
  ...
  private void evenSolution() {
    while (true) {
      ...
      if (board.legalMove(Board.PegA,Board.PegB))
        board.move(Board.PegA,Board.PegB);
      else {
        if (!board.legalMove(Board.PegB,Board.PegA))
          throw new IllegalStateException();
        board.move(Board.PegB,Board.PegA);
      }
      nmoves++;
      if (ps!=null) board.echo(ps);
      if (completed())
      ...
      if (board.legalMove(Board.PegA,Board.PegC))
        board.move(Board.PegA,Board.PegC);
      else {
        if (!board.legalMove(Board.PegC,Board.PegA))
          throw new IllegalStateException();
        board.move(Board.PegC,Board.PegA);
      }
      nmoves++;
      if (ps!=null) board.echo(ps);
      if (completed())
      ...
      if (board.legalMove(Board.PegB,Board.PegC))
        board.move(Board.PegB,Board.PegC);
      else {
        if (!board.legalMove(Board.PegC,Board.PegB))
          throw new IllegalStateException();
        board.move(Board.PegC,Board.PegB);
      }
      nmoves++;
      if (ps!=null) board.echo(ps);
      if (completed())
